...
export function getEmailProvenance(email: Email): ProvenanceDetails {
  ...
  const isVerified = !isSmtpBridge && (["verified", "priority", "encrypted", "receipts", "inbox"].includes(email.folder) || !!email.senderPolicy);
  ...
  const senderIdentityInspector: ProvenanceItemDetails = {
    ...
    description: "Detailed trace of how the sender's identifier was mapped to their cryptographic public key and verified.",
    keyValuePairs: [
      ...
      { label: "Status", value: isVerified ? "Cryptographically Verified" : "SMTP Unsigned / Bridged" },
      { label: "Federation URL", value: rawIdentity.includes("*") ? `https://${rawIdentity.split("*")[1]}/.well-known/stellar.toml` : "N/A" }
    ],
    rawJson: JSON.stringify({
      ...
      verification_trace: [
        { step: 1, action: "Lookup domain stellar.toml", status: rawIdentity.includes("*") ? "OK" : "SKIP" },
        { step: 2, action: "Resolve account alias", status: "OK", result: resolvedKey },
        { step: 3, action: "Validate cryptographic envelope signature", status: isVerified ? "VALID" : "MISSING" }
      ]
    }, null, 2)
  };
  ...
  const relayTimestamp = email.time.includes("AM") || email.time.includes("PM")
  ...
  const relaySourceInspector: ProvenanceItemDetails = {
    ...
    description: "The decentralised relay node that processed, routed, and committed this message to the network.",
    keyValuePairs: [
  ...
  const messageHashInspector: ProvenanceItemDetails = {
    ...
    description: "The cryptographic SHA-256 digest of the message plaintext used to verify that the email body has not been altered in transit.",
    keyValuePairs: [
  ...
  const payloadCommitmentInspector: ProvenanceItemDetails = {
    ...
    description: "The commitment hash registered on-chain. It proves the message was sent at a specific time without revealing its encrypted contents to the public ledger.",
    keyValuePairs: [
  ...
  const postageAmount = isSmtpBridge ? "0.00000 XLM (No postage)" : isRequest ? "0.00500 XLM" : "0.00001 XLM";
  ...
  const postageRecordInspector: ProvenanceItemDetails = {
    ...
    description: "Proof of anti-spam postage payment submitted on the Stellar ledger to purchase delivery priority.",
    keyValuePairs: [
  ...
  const receiptRecordInspector: ProvenanceItemDetails = {
    ...
    description: "Smart contract state confirming the message was delivered and cryptographically acknowledged by the recipient's client.",
    keyValuePairs: [
